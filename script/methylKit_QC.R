...
for (context in context_list) {
  ...
  file_list <- as.list(paste0('output/', config$samples, '_clean_bismark_bt2.deduplicated.', context, '_report.txt.gz'))
  raw <- methRead(file_list, sample.id=as.list(config$samples), assembly=config$assembly, treatment=config$treatment, 
